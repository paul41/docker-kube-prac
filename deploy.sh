...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=stephnegrider/multi-server:$SHA
kubectl set image deployments/client-deployment client=stephnegrider/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=stephnegrider/multi-worker:$SHA